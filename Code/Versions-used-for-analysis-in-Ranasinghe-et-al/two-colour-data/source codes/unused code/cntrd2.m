function out=cntrd(im,mx,sz,interactive,im2)
...
for i=1:nmx
    ...
    tmp2=msk.*im2((mx(i,2)-r+1:mx(i,2)+r),(mx(i,1)-r+1:mx(i,1)+r));
    %calculate the total brightness
    norm2=sum(sum(tmp2))+0.2;
    norm = sum(sum(tmp));
    ...
    %concatenate it up
    pts=[pts,[mx(i,1)+xavg-r,mx(i,2)+yavg-r,norm2,rg]'];
