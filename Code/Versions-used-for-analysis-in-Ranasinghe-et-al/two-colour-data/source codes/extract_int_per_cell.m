function [xyAreaInt] = extract_int_per_cell(varargin)
...
STATS = regionprops(L,I,'Area','MeanIntensity','Centroid','PixelIdxList','Perimeter','FilledImage');
...
xyAreaInt = [aPOS aAREA aINT aINTsubBG aAXIS OFF'];
...
if parameters.display_images == 1 && strcmp(type,'r')
    ...
    for k=1:length(B2)
        ...
        h = text(col+1, row-1, num2str(k));%k,7)));%6)/xyAreaInt(k,3)));
